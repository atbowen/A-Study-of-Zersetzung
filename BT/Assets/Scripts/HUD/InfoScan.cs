...
public class InfoScan : MonoBehaviour {
    ...
    public void LookingAtNote(ID ident) {
        IDDocument targetedID = (IDDocument)ident;
    ...
    public void DisplayBoundingBoxCorners() {
        ...
        if (switchingColliders && IDThing != null) {
            ...
            if (parentOfID.GetComponent<Collider>() != null) {
                colliderFound = true;
                Collider coll = parentOfID.GetComponent<Collider>();
            ...
            } else if (parentOfID.GetComponent<CapsuleCollider>() != null) {

                colliderFound = true;
                CapsuleCollider coll = parentOfID.GetComponent<CapsuleCollider>();
                boundBoxCenter = coll.bounds.center;
        ...
        if (colliderFound) {
            ...
            for (int i = 0; i < bbRend.Length; i++) {
                ...
                bbRend[i].positionCount = 2;
                bbRend[i].SetWidth(segWidth, segWidth);
            }

            bbRend[0].SetPosition(0, corner1);
        ...
        if (currentID != null) {
            ...
            if (currentID.GetType() == typeof(IDInteractable)) {
                ...
                if (IDThing.switchColliderWithPointerColliders && IDThing.currentSwitchCollider != null) {
                    ...
                    col = IDThing.transform.parent.GetComponent<Collider>();
                }
            }
            else { col = currentID.transform.parent.GetComponent<Collider>(); }
    ...
    private void SetBBColor(ID IDThing) {
        ...
        float distance = Vector3.Distance(bCam.transform.position, IDThing.transform.parent.GetComponent<Collider>().ClosestPoint(bCam.transform.position));
